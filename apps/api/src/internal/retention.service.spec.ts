...
describe('RetentionService', () => {
  it('deletes only selections older than each user retention window', async () => {
    ...
    const prisma = {
      user: { findMany: vi.fn().mockResolvedValue([{ id: 'user-1', timezone: 'Asia/Ho_Chi_Minh', historyRetentionDays: 30 }]) },
    ...
    expect(deleteMany).toHaveBeenCalledWith({ where: { userId: 'user-1', localDate: { lt: '2026-08-15' } } });
