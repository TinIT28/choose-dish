...
import { listDishes, listSharedDishes } from '../features/dishes/api';
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section aria-label="Danh sách món ăn">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {dishesQuery.data?.map((dish) => <DishCard key={dish.id} dish={dish} />)}
          </div>
        </section>
        <DishForm accessToken={accessToken} onSaved={() => void dishesQuery.refetch()} />
      </div>
