...
The application uses a Bun workspace monorepo with `apps/web` and `apps/api`, deployed as two Vercel projects from the same repository. The React/Rsbuild app is deployed as the frontend and the NestJS app runs as a Vercel Function; Neon provides PostgreSQL and Cloudinary provides image storage, keeping the deployable units independent while avoiding a long-running server requirement.
